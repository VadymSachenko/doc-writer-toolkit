...
| Мітка пункту списку (термін до двокрапки) | **жирний** до двокрапки, решта — звичайний текст | **Картка:** поля `CardNumber` і `CardCoded` заповнені | **Card:** the `CardNumber` and `CardCoded` fields are filled |
...
вкладку, підпис поля в адмінці) **і мітку пункту списку** (термін, що визначається, у форматі
`- **Термін:** пояснення`). Це два дозволені випадки, і за їхні межі жирний не виходить. Для
мітки UI написання зберігається дослівно таким, як його показує інтерфейс.

- **Жирний як емфаза — заборонено.** Ані для «важливого» речення, ані для назви модуля чи
  фічі. Якщо факт важливий — винеси його в окреме речення або в `:::note`, а не виділяй.
- **Мітка пункту списку — структурний виняток, не емфаза.** У пункті списку, що визначає
  термін (`- **Термін:** пояснення`), жирний покриває **тільки термін до двокрапки**; сама
  двокрапка і решта пункту — звичайний текст. Це той самий принцип, що й мітка UI: жирний
  позначає структурну роль (тут — заголовок пункту), а не важливість вмісту.
  Приклад: `- **Картка:** поля CardNumber і CardCoded заповнені; поле Link порожнє.` Якщо
  жирним виділено більше за термін до двокрапки — увесь пункт, речення без структури
  «термін: пояснення» — це вже емфаза, і вона заборонена (перший пункт вище).
...
**ніколи** — заборона жирного як емфази беззастережна, винятків немає. Мітка пункту списку
(вище) не є винятком із цієї заборони: вона взагалі не емфаза, а структурна роль, як і мітка
UI, — тому й підпадає під той самий рендер.
...
| **Картка, IBAN, посилання, квазі, телефон: заповнене тільки поле `Link`.** (жирний на весь пункт — емфаза) | **Картка, IBAN, посилання, квазі, телефон:** заповнене тільки поле `Link`. (жирний лише до двокрапки — мітка пункту) |
...
- [ ] Жодного жирного, який не є видимою міткою UI або міткою пункту списку (Ж1)?
- [ ] Жодного жирного «для важливості» (Ж1)?
- [ ] У мітках пунктів списку жирний не заходить за двокрапку (Ж1)?
